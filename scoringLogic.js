...
try {
  console.log(`[PREHIRE AIRTABLE] Starting batch save for ${employees.length} employees`);
console.log(`[PREHIRE AIRTABLE] AIRTABLE_API_KEY set: ${!!process.env.AIRTABLE_API_KEY}`);
console.log(`[PREHIRE AIRTABLE] AIRTABLE_BASE_ID: "${process.env.AIRTABLE_BASE_ID}"`);
console.log(`[PREHIRE AIRTABLE] AIRTABLE_PREHIRE_EMPLOYEE: "${process.env.AIRTABLE_PREHIRE_EMPLOYEE}"`);


if (!process.env.AIRTABLE_API_KEY || !process.env.AIRTABLE_BASE_ID || !process.env.AIRTABLE_PREHIRE_EMPLOYEE) {
  console.error(`[PREHIRE AIRTABLE] ❌ Missing required env var(s) — skipping Airtable save, continuing without it`);
  return;
}
...
for (let i = 0; i < records.length; i += 10) {
const chunk = records.slice(i, i + 10);
console.log(`[PREHIRE AIRTABLE] Sending chunk starting at ${i}, size ${chunk.length}...`);
const timeoutPromise = new Promise((_, reject) =>
  setTimeout(() => reject(new Error('Airtable table.create timed out after 15s')), 15000)
);

try {
  await Promise.race([table.create(chunk), timeoutPromise]);
  console.log(`✅ Airtable (PreHire) batch ${i + 1}–${i + chunk.length}`);
} catch (chunkError) {
  console.error(`[PREHIRE AIRTABLE] ❌ Chunk ${i} failed/timed out:`, chunkError.message);
}
